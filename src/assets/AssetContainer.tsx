...
let { sendAssets, syncAsset, loadMoreCards } = Redux.routines
...
function Container(_props: MappedProps & {
  actions: Record<'send' | 'triggerSyncAsset' | 'stopSyncAsset' | 'loadMoreCards', any>
}){
  if(((p: any): p is {redirect: true} => Boolean(p.redirect))(_props)){
    return <Redirect to='/assets' />
  }
  let { stage, actions: { triggerSyncAsset, stopSyncAsset, ...actions }, ...props } = _props
  return <Asset
    sync={{
      stage,
      trigger: () => triggerSyncAsset({
        asset: props.asset,
...
export default connect(
  ...
  (dispatch: Dispatch<any>) => ({
    actions: bindActionCreators({
      ...
      loadMoreCards: loadMoreCards.trigger,
    },
    dispatch
  )})
)(Container)
